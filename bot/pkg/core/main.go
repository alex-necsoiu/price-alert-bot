...
func main() {
	...
	for {
		...
		fmt.Printf("### ¿Would you like to enter another Currency Pair? {Replay: 'yes'/'no'}: ")
		scanner.Scan()
		fmt.Printf("\n\n")
		...
		if strings.ToUpper(scanner.Text()) != "NO" || strings.ToUpper(scanner.Text()) != "YES" {
			fmt.Printf("### Invalid Input ¿Would you like to enter another Currency Pair? Replay: yes/no: ")
			scanner.Scan()
			fmt.Printf("\n\n")
		}
	...
	// Call The Ticker Function with the given
